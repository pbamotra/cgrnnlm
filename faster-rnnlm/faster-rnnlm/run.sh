...
then
    exe=./rnnlm 
 $exe -rnnlm $basedir/models/$mname -train $data/ptb.train.txt -valid $data/ptb.valid.txt -hidden $hidden_size --context_size $context_size --retry $retry -threads 1 2>&1 | tee f.out
